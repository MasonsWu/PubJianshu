...
class r_content(object):
    ...
    def getrandomline(filename):
        """读取文件的任意一行"""
        import random
        return r_content.readtline(filename,random.randint(0, r_content.getfilelines(filename)),).decode('gbk')


# if __name__ == "__main__":
#     if len(sys.argv) == 1:
#         print(r_content.getrandomline("content.csv"))
#     else:
#         for f in filter(os.path.isfile, sys.argv[1:]):
#             print(r_content.getrandomline(f))
